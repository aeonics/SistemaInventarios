...
$conexion = dbConnect();
$result_eo = $conexion->prepare('SELECT * FROM EquipoOficina WHERE id_sistema = :id_eo LIMIT 1');
$result_eo->execute(array(':id_eo' => $id_eo));
$rows_eo = $result_eo->fetch();
if ($rows_eo != false) {
...
if ($error == ''){
$result_eo = $conexion->prepare('INSERT INTO EquipoOficina (id_sistema, Ubicacion_idUbicacion, Proveedores_idProveedores, Empleado_idEmpelado, Identificador, Marca, Modelo, Serie, Estatus, Descripcion, Costo, Iva, FechaCompra, RutaImagen, Factura, XML ) VALUES (:id_eo, :ubicacion, :proveedor, :responsable, :identificador, :marca, :modelo, :serie, :estatus, :descripcion, :costo, :iva, :fechacompra, :rutaimagen, :pdf, :filexml )') or die(mysql_error());
$result_eo->execute(array(
':id_eo' => $id_eo,
':ubicacion' => $ubicacion,
':proveedor' => $proveedor,
...
//////////// SE ACTUALIZA SIEMPRE Y CUANDO NO HAYA ERRORES
  ...
  if ($error=="") {
    $query="UPDATE EquipoOficina SET Antiguedad ='$calc_edad', ValorActual='$valoractual' WHERE id_sistema ='$id_eo'";
    $result = $conexion->query($query);
    $Aviso="<li>Ahora puedes visualizar el Codigo QR</li>";
  }

}
require ('vista/equipo_oficina.view.php');
}
else {
